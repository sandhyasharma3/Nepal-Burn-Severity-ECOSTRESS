...
# minimum n for stratified models
min_n_lc      <- 400
min_n_topo    <- 50
min_n_lc_topo <- 50
...
# land cover codes from MODIS MCD12Q1
lulc_codes <- c(
...
# Okabe-Ito colour palette for figures
okabe_ito <- c(
  "Spatial"      = "#0072B2",
  "Hydrological" = "#009E73",
  "Topographic"  = "#D55E00"
...
# -----------------------------------------------------------------------------
# load PCNM vectors
# PCNMs were calculated on all 40,000 points before NA removal
# this ensures eigenvectors capture the full spatial extent of the grid
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# load rasters and extract values at sampling points
# -----------------------------------------------------------------------------
...
names(rstack) <- c("Aspect_deg", "Elevation", "Slope", "dNBR", "LC",
                   "ESI_Annual", "ESI_SS", "ET_Annual", "ET_SS",
                   "WUE_Annual", "WUE_SS")

# reproject points if CRS does not match
...
# categorise topographic variables
...
# north wraps around 0/360 so fix it
...
# filter by dNBR threshold and rename
...
# remove rows with NA in any predictor column
hydro_vars <- c("ESI_Annual", "ESI_SS", "ET_Annual", "ET_SS",
                "WUE_Annual", "WUE_SS")
...
imp_df$Label[imp_df$Variable == "ESI_Annual"]  <- "ESI (Annual)"
imp_df$Label[imp_df$Variable == "ESI_SS"]      <- "ESI (Spring-Summer)"
imp_df$Label[imp_df$Variable == "ET_Annual"]   <- "ET (Annual)"
imp_df$Label[imp_df$Variable == "ET_SS"]       <- "ET (Spring-Summer)"
imp_df$Label[imp_df$Variable == "WUE_Annual"]  <- "WUE (Annual)"
imp_df$Label[imp_df$Variable == "WUE_SS"]      <- "WUE (Spring-Summer)"
...
fig3 <- (p_imp + labs(tag = "A")) | (p_pred + labs(tag = "B"))

ggsave(file.path(out_dir, "Figure3.tiff"), fig3,
       device = "tiff", dpi = 600, width = 14, height = 7,
       units = "in", compression = "lzw")

ggsave(file.path(out_dir, "Figure3.png"), fig3,
       dpi = 300, width = 14, height = 7, units = "in")

# =============================================================================
# stratified RF by land cover class
# predictors = ECOSTRESS hydro vars + top 3 PCNMs
# performance assessed via OOB R2 only (no train/test split)
# =============================================================================
...
write.csv(lc_table, file.path(out_dir, "Table1_LC_stratified.csv"), row.names = FALSE)
print(lc_table)
...
write.csv(topo_table, file.path(out_dir, "Table2_Topo_stratified.csv"), row.names = FALSE)
print(topo_table)

# =============================================================================
# stratified RF by land cover x topographic class (Table S1 - Supplement)
# =============================================================================
...
write.csv(lc_topo_table, file.path(out_dir, "TableS1_LCxTopo.csv"), row.names = FALSE)

cat("LC x Topo done. Top rows:\n")
